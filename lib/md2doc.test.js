...
describe('md2doc', () => {
  ...
  describe('when there are markdown and html files with the same name', () => {
    ...
    it('log a warning', () => {
      ...
      expect(logger.warn.mock.calls[0][0]).toBe("Already exist a file with the same name: 'doc/file.md'.");
    });
  });
  ...
  //     expect(fs.readFileSync('site/index.html', 'utf8')).toBe('files content');
