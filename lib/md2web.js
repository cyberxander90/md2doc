...
const md2Web = (config) => {
  ...
  if (indexHtml) {
    mdFilePaths.forEach((filePath) => {
      const indexHtmlParser = new HtmlParser({ html: indexHtml });
      const outFilePath = filePath.replace(/md$/, 'html');
      const fileHtml = getFileContent(outFilePath);
      indexHtmlParser
        .addElement('#table-of-contents', generateTableOfContents(fileHtml))
        .addElement('#site-nav', siteNavHtml)
        .addElement('#main', fileHtml)
        .saveTo(outFilePath);
    });

    const indexHtmlParser = new HtmlParser({ html: indexHtml });
    indexHtmlParser
      .removeElement('#table-of-contents')
      .addElement('#site-nav', siteNavHtml)
      .addElement('#main', 'Dev Docs')
      .saveTo(indexHtmlPath);
  }
};
