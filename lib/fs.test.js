...
describe('fs', () => {
  ...
  describe('#copyFile', () => {
    const subject = copyFile;

    describe('when already exist the file destination', () => {
      beforeEach(() => {
        mockFs({
          'path/to/dir/file.txt': 'file content',
          'path/destination/index.txt': 'hello world',
        });
        subject('path/to/dir/file.txt', 'path/destination/index.txt');
      });
    ...
    describe('when the destination file do not exist', () => {
      ...
      it('creates the file with the content', () => {
        expect(fs.readFileSync('path/to/dir/file.txt', 'utf-8')).toBe('file content');
        expect(fs.readFileSync('path/destination/index.txt', 'utf-8')).toBe('file content');
      });
    });
  });
